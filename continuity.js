(function(){
  ...
  window.setLanguage=function(selected){if(typeof originalSetLanguage==='function')originalSetLanguage(selected);updateButton()};
  function firstInteraction(e){if(e.target.closest&&e.target.closest('#audio-btn'))return;if(localStorage.getItem(AUDIO_PREF)!=='on')return;play(false);removeEventListener('pointerdown',firstInteraction,true);removeEventListener('keydown',firstInteraction,true)}
  if(audioEl){audioEl.volume=.46;restore();audioEl.addEventListener('play',updateButton);audioEl.addEventListener('pause',updateButton);setInterval(()=>{if(!audioEl.paused)save()},1500);const pref=localStorage.getItem(AUDIO_PREF);if(pref==='on')play(false).then(ok=>{if(!ok){addEventListener('pointerdown',firstInteraction,true);addEventListener('keydown',firstInteraction,true)}});updateButton()}
